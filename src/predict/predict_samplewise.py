...
def predict_byte(model, partition, xfiles, args):
    ...
    # tet = time.time()
    # print("[][][][][][][][][][][][][][][][][][][][][][][1] SAMPLE-WISE  Tier-1 Prediction Time :", int((tet - tst)*1000)/1000, "ms")
    return pred


def predict_byte_by_section_by_samples(model, xfiles, q_sections, args):
    """
    ...
    #tst = time.time()
    pred = model.predict(
        utils.data_generator_by_section_by_samples(q_sections, xfiles, np.ones(xfiles.shape), args.max_len, args.batch_size, shuffle=False),
        steps=pred_steps,
        verbose=args.verbose
        )
    #tet = time.time()
    #print("[][][][][][][][][][][][][][][][][][][][][][][2] SAMPLE-WISE Tier-2 Prediction Time :", int((tet - tst)*1000)/187, "ms")
    return pred
...
def predict_byte_by_section(model, spartition, xfiles, q_sections, section_map, args):
    ...
    tet = time.time()
    print("[][][][][][][][][][][][][][][][][][][][][][][3] SAMPLE-WISE  Tier-2 Prediction Time :", int((tet - tst) * 1000) / 187,
          "ms")
    return pred
...
def predict_tier2_by_samples(model_idx, pobj, fold_index):
    """
    ...
    if cnst.EXECUTION_TYPE[model_idx] == cnst.BYTE:
        # pbs.trigger_predict_by_section()
        pobj.yprob = predict_byte_by_section_by_samples(tier2_model, pobj.xtrue, pobj.q_sections, predict_args)
    elif cnst.EXECUTION_TYPE[model_idx] == cnst.FEATURISTIC:
        pobj.yprob = predict_by_features(tier2_model, pobj.xtrue, predict_args)
    elif cnst.EXECUTION_TYPE[model_idx] == cnst.FUSION:
        pobj.yprob = predict_by_fusion(tier2_model, pobj.xtrue, predict_args)
...
def reconcile(pt1, pt2, cv_obj, fold_index):
    ...
    try:
        ...
        reconciled_df.to_csv(cnst.PROJECT_BASE_PATH+cnst.ESC+'out'+cnst.ESC+'result'+cnst.ESC + "overall_pred_results_" + str(fold_index) + ".csv", header=None, index=None, mode='a')
        ...
        ftier2_df.to_csv(cnst.PROJECT_BASE_PATH+cnst.ESC+'out'+cnst.ESC+'result'+cnst.ESC + "tier2_test_" + str(fold_index) + ".csv", header=None, index=None, mode='a')
...
def init(model_idx, test_partitions, cv_obj, fold_index, scalers=None):
    ...
    if not cnst.SKIP_TIER1_PREDICTION:
        ...
        print("[][][][][][] FOR ALL PARTITIONS [SAMPLE-WISE] Tier-1 Prediction Time :", int(total * 1000) / 1000, "ms")
    ...
    if thd2 is not None and q_sections is not None:
        ...
        predict_t2_test_data_partition = pObj(cnst.TIER2, None, predict_t1_test_data_all.xB1, predict_t1_test_data_all.yB1)  #test_b1datadf_all.iloc[:, 0], test_b1datadf_all.iloc[:, 1])
        ...
        tst = time.time()
        predict_t2_test_data_partition = predict_tier2_by_samples(model_idx, predict_t2_test_data_partition, fold_index)
        total = time.time() - tst
        print("[][][][][][] FOR ALL SAMPLES [SAMPLE-WISE] Tier-2 Prediction Time :", int(total * 1000) / 187, "ms")
